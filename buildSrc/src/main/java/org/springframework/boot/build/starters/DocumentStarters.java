...
public class DocumentStarters extends DefaultTask {
    ...
    void documentStarters() {
        Set<Starter> starters = this.starters.getFiles().stream().map(this::loadStarter).collect(Collectors.toCollection(TreeSet::new));
    ...
    private Starter loadStarter(File metadata) {
        ...
        try (FileReader reader = new FileReader(metadata)) {
            ...
            return new Starter(properties.getProperty("name"), properties.getProperty("description"), StringUtils.commaDelimitedListToSet(properties.getProperty("dependencies")));
    ...
    private static final class Starter implements Comparable<Starter> {
        ...
        private boolean isTechnical() {
            return !Arrays.asList("spring-boot-starter", "spring-boot-starter-test").contains(this.name) && !isProduction() && !this.dependencies.contains("spring-boot-starter");
        }
